...
class DCPM(DSP, ClassifierMixin):
    ...
    def __init__(
        self, n_components: int = 1, transform_method: str = "corr", online_flag: bool = False
    ):
        self.n_components = n_components
        self.transform_method = transform_method
    ...
    def fit(self, X: ndarray, y: ndarray):  # type: ignore[override]
        ...
        if self.online_flag:
            self._prepare_template_cache()
        return self

    def _prepare_template_cache(self):
        templates = np.reshape(self.templates, (-1, *self.templates.shape[-2:]))
        templates = templates - np.mean(templates, axis=-1, keepdims=True)
        templates = np.reshape(templates, (templates.shape[0], -1))
        self.templates_prepare = templates
        self.istd_templates = 1 / np.std(templates, axis=-1, keepdims=True)
        self.templates_len = templates.shape[1]

    def _pearson_features(self, X: ndarray):
        X = np.reshape(X, (-1, *X.shape[-2:]))
        X = X - np.mean(X, axis=-1, keepdims=True)
        X = np.reshape(X, (X.shape[0], -1))
        istd_X = 1 / np.std(X, axis=-1, keepdims=True)
        corr = (X @ self.templates_prepare.T) / (self.templates_len - 1)
        corr = istd_X * corr * self.istd_templates.T
        return corr
    ...
    def transform(self, X: ndarray):
        ...
        if self.online_flag:
            feature = self._pearson_features(X_feature)  # feature(n_trials, n_classes)
        else:
            feature = super()._pearson_features(X_feature, self.templates)  # feature(n_trials, n_classes)
        return feature
